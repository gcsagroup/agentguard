...
状态：**整体仍为设计候选；已实现默认关闭的实验性 Chrome/Edge 网页邮箱模块，尚未实现邮箱代理或接入真实邮箱**。以下网关、客户端和批准流程仍是计划，不代表网页模块已具备这些能力。

## 当前可试用范围（2026-09-07）

入口：加载最新 Chromium 扩展后，打开扩展弹窗 →「邮件设置」→「启用实验性网页邮箱检查」。初始关闭；开关仅控制邮件专项，不关闭既有付款防护。界面报告候选页面与检查状态，不声称实际投递路径已验证。

- 候选地址限定为 HTTPS 的 `mail.google.com/mail/`、`outlook.live.com/mail/`、`outlook.office.com/mail/` 和 `outlook.office365.com/mail/`。已识别的发送点击、快捷键和表单事件会核对主题、正文及 To/Cc/Bcc；密码、部分令牌/私钥、通过校验的卡号和典型提示注入会触发阻断。它不是完整 DLP，不猜测企业内外域。
- 阅读区检查典型伪指令，点击时检查已识别链接的协议及显示域名错配；这不是恶意网址情报或邮件真伪认证。Unicode 仅在匹配视图归一化，原文不改写。
- **未实现附件内容扫描**：开启后阻断已识别的文件选择、拖放、文件粘贴，以及携带已识别附件或内嵌媒体的发送；邮箱页其他文件控件也可能被拦。复杂地址、未知主题/正文结构或超出文本预算时保守阻断。
- 仅本地检查；邮件专项记录只保存风险类别、阻断状态、时间和站点来源，不保存正文、主题、邮箱地址、附件名或完整链接，不连接邮箱 API，也不增加权限或 Native Messaging。默认关闭时不运行邮件专项阅读扫描。
- 页面提示只有「关闭」，**没有批准、一次放行或自动发送**。关闭提示不发送邮件。已在该模块开启前提交或自动同步的内容不会撤回。
- **明确缺口**：直接 API、草稿自动同步、未知控件/附件路径、未注入 frame、closed Shadow DOM、桌面客户端和其他邮箱不在本版保证内。真实 Gmail/Outlook 页面以及 Edge 尚待独立验收；目前仅用原样扩展、真实 Chromium、本地合成 DOM 与接收端计数验证已声明事件。

复现：Node 22 下执行 `make check-extension-gate`、`make e2e-extension`、`make ui-preview`；后两项需要 Playwright/Chromium。网页用例为 `WM-*`；其中 `*-GAP` 故意证明直接 API 可以绕过，不能计为防护成功。原有 MAIL-01–10 是下文整体方案的验收计划，不因本模块而自动通过。
