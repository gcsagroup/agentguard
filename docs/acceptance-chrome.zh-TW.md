...
- `make e2e-extension` 在測試 Chromium 中執行 41 條基礎判據及 54 條實驗性郵件檢查，共 95 條，核對限定範圍內的阻擋行為與已知缺口；
...
`make e2e-extension` 把擴充功能裝入真 Chromium 持久化情境，輸出 `eval/e2e-extension/out/report.json` 與唯一結論標記 `AGENTGUARD_E2E_EXTENSION=PASS|FAIL`。目前 41 條基礎案例分組如下；另有 54 條實驗性郵件檢查使用合成 Gmail／Outlook DOM，包含直接 API 可繞過 DOM 門的邊界複核，不證明真實郵件服務或 Edge 已驗收。
...
| F3a–F3e | 隱私陷阱表單、腳本 `form.submit()` 與一般子 frame 付款動作在導覽/POST 前阻擋；關閉提示後仍不送出 |
| F4a–F4e / H2–H4 | fetch、XHR、sendBeacon、form、明確編碼與操作查詢由 DNR 在伺服器前阻擋；舊 decision/scope 訊息與舊 15 秒逾時不能放行 |
| F5a–F5e | GET、一般 POST、僅 body 付款語意、付款前綴一般詞、巢狀查詢文字與一般 `form.submit()` 不誤擋 |
...
成功必須同時滿足：程序退出 0、最後一行為 `AGENTGUARD_E2E_EXTENSION=PASS`、`report.json` 的 `all_pass` 為 `true`，且報告記錄目前清單預期的 95 條 PASS（41 條基礎、54 條實驗性郵件檢查）。其中已知繞過案例的 PASS 表示缺口仍可重現，不表示防護成功。必須保留報告中的 Chromium 版本；不得改稱 Chrome 或 Edge 正式版證據。
...
| B4 | 從上一公開版本原地升級到同一候選 ZIP | 不新增 Native 權限；舊暫停、舊動態 host 規則與舊徽章不殘留；41 條自動化涵蓋的代表性阻擋仍成立 | 升級前後權限、storage/規則、popup |
...
- DOM 保證涵蓋擴充功能實際注入的 HTTP(S) frame：可觀察的 `click` / `submit`，以及未改寫原型的 `form.submit()`。頁面從 iframe 恢復原生方法、未注入的特殊 frame、pointer/keyboard 自訂前置邏輯與瀏覽器外原生動作不在保證內。
