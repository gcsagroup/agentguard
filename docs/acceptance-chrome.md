...
| # | 步骤 | 期望 | 实测 | 证据 |
|---|------|------|------|------|
| C1–C5 | `make e2e-extension` | 最后一行 `AGENTGUARD_E2E_EXTENSION=PASS`，`report.json` 里 `all_pass: true`；把 `out/report.json`、`out/f2-payment-dialog.png`、`out/popup.png` 复制到 `evidence/chrome/` | | |
