...
用户指出原提交后 Codex 仍显示约 15,276 行变更。核对后，15,240 行来自 `intel/candidates/automation/` 的 91 份公开来源研究、候选和运行状态，另 36 行是 `docs/agent-threat-intel-automation.zh.md`；这些文件原属另一项任务，前一提交按范围排除，因而仍显示为未跟踪。本次按用户要求作为独立后续提交纳入 `main`，保留“候选未签名、未发布、生效规则包未改”的界限。所有 JSON 均完成解析检查，文件中未检出密钥或凭据字段。Python `__pycache__` 属本机编译缓存，加入 `.gitignore`，文件本身保留在本机。
