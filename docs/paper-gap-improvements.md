...
| AX auto-poll (macOS desktop) | Historical iter-3 implementation: `ax_auto_cmd(enable)` + 2.5s background poller. Superseded on 2026-09-01 by the product-wired AXObserver driver (50ms tick, 150ms debounce, 800ms ceiling, 3s fallback); see [macos实时观测.md](./macos实时观测.md). |
