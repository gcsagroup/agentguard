...
AgentGuard 的 `--strict` **RC 技术门禁**不再把“某个文件里出现了关键词”当作证据。需要凭据或真机的十二类检查必须提供结构化 JSON，并绑定**当前完整提交号**以及现场重算的产物摘要：普通发布文件使用标准 SHA-256，Apple `.app` 使用整个 bundle 的确定性 tree-v2，验收报告使用包含报告与逐项材料的 acceptance-closure-v1。

> `--strict` 通过只表示候选可进入 GA 收口，不是 GA 批准或已上线。`--ga` 在这 12 类之上再要求 7 类闭环证据，共 19 类；见《[GA 发布闭环门禁](ga-release-gate.md)》。
...
| `ios_codesign` | 最终设备 `.app` 通过 codesign 严格验证，输出含 Apple Distribution authority 与预期 Team ID | `AGENTGUARD_EVIDENCE_IOS_CODESIGN` |
...
| `acceptance_ios` | iPhone/iPad 上 I1–I6 Safari Web Extension 真机清单完成 | `AGENTGUARD_EVIDENCE_ACCEPTANCE_IOS` |
| `acceptance_ios_testflight` | TF1–TF3 上传、真机安装与升级完成 | `AGENTGUARD_EVIDENCE_ACCEPTANCE_IOS_TESTFLIGHT` |
| `acceptance_chrome` | Chrome stable 上候选 ZIP 的 B1–B5 完成 | `AGENTGUARD_EVIDENCE_ACCEPTANCE_CHROME` |
| `acceptance_edge` | Edge stable 上同一候选 ZIP 的 B1–B5 独立完成 | `AGENTGUARD_EVIDENCE_ACCEPTANCE_EDGE` |
...
- 五类签名证据的 `signer` 必须是非空字符串，并与门禁从证据 JSON 之外取得的预期发布者身份一致；严格门禁使用的七类验收证据之 `signer` 必须是 `null`，且直接调用验证器时禁止传 `--expected-signer`。
...
- `artifact.path` 必须是仓库根目录下的相对路径，只用 `/` 分隔；每个组件必须匹配可移植 ASCII `[A-Za-z0-9._-]+`，且不能是 `.` 或 `..`。绝对路径、反斜线、空白、shell glob/展开字符和控制字符都会被拒；目标及其任一路径组件也不能是符号链接。普通文件必须非空；macOS/iOS 签名 kind 还接受至少包含一个非空普通文件的 `.app` 目录，但 bundle 内任一符号链接、特殊文件或非 UTF-8 路径都会 fail-closed。
...
- 五类签名证据的 `command` 必须无 `#`、PowerShell 或 batch 注释，并采用校验器接受的精确、fail-closed 成功链。macOS/iOS 代码签名必须是 `codesign --verify --deep --strict --verbose=4 ARTIFACT && codesign -dv --verbose=4 ARTIFACT`；iOS 输出还必须证明 Apple Distribution authority。Windows 必须先执行 `signtool verify /pa /v` 并检查 PowerShell `$?` 与 `$LASTEXITCODE`，再对同一文件执行 `Get-AuthenticodeSignature`、验证状态/证书并现场输出指纹；Android 只接受单段 `apksigner verify --print-certs ARTIFACT`。产物必须位于 `evidence/`、`dist/`、`target/release/`，或 `apps/**/dist/`、`apps/**/target/release/`、`apps/**/build/outputs/` 的发布产物路径；`macos_codesign` 只接受 `.app`/`.dmg`，`macos_notarize` 接受 `.app`/`.dmg`/`.pkg`，`ios_codesign` 只接受 `.app`，Windows 接受 `.exe`/`.msi`/`.msix`，Android 只接受 `.apk`。
...
- 签名类 `output` 除成功信号外还必须绑定证书身份：codesign 包含 `valid on disk`、`satisfies its designated requirement` 与精确行 `TeamIdentifier=<10 位 Team ID>`；iOS 还包含 `Authority=Apple Distribution:`；notarytool 包含 Accepted 状态且 stapler 包含 `validate action worked`；Windows 包含 `Successfully verified` 与精确行 `CertificateSHA256=<64 位十六进制摘要>`；Android 包含 `Signer #1 certificate SHA-256 digest: <证书指纹>`。Android 输出若出现 `Android Debug` 会被拒绝。
- 严格门禁使用的七类验收证据分别只接受对应 `evidence/<平台>/` 前缀下、大小不超过 16 MiB 的 `.md` 报告：macOS、Android、Windows、iOS、iOS TestFlight、Chrome、Edge。`command` 必须是实际成功执行的单段 `guard-cli manual-acceptance <平台> <清单> <artifact.path> --repo-root .`；固定清单见上表和验收文档。
- 校验器会解析 Markdown 表：Windows 为 W1–W6/W8–W11；Android 为 A1–A4；macOS 为 1、2、3、4、5、5b、5c 与 6–18；iOS 为 I1–I6；TestFlight 为 TF1–TF3；Chrome 与 Edge 各为 B1–B5。每个必需 ID 必须恰好一行，第二列精确为 `PASS (native)`，第三列必须位于该 kind 对应的 `evidence/<平台>/` 目录且不得复用。缺失、重复、`PASS (sim)`、FAIL、BLOCKED 或 N/A 都会被拒。报告正文和 JSON 的 `output` 还必须包含该 kind 的精确成功 marker。
...
严格门禁使用的验收标记为 `AGENTGUARD_ACCEPTANCE_MACOS=PASS`、`AGENTGUARD_ACCEPTANCE_ANDROID=PASS`、`AGENTGUARD_ACCEPTANCE_WINDOWS=PASS`、`AGENTGUARD_ACCEPTANCE_IOS=PASS`、`AGENTGUARD_ACCEPTANCE_IOS_TESTFLIGHT=PASS`、`AGENTGUARD_ACCEPTANCE_CHROME=PASS` 与 `AGENTGUARD_ACCEPTANCE_EDGE=PASS`。

### Firefox 遗留格式不是首个 GA 证据

`guard-cli` 暂时仍能解析 `acceptance_firefox`，仅用于旧版/未来格式兼容。`scripts/release-gate.sh --strict` 不读取 `AGENTGUARD_EVIDENCE_ACCEPTANCE_FIREFOX`，本文件也不提供生成 Firefox PASS 的发布流程。直接调用遗留命令得到的标记不能改变“Firefox 仅源码原型、不打包、不提交、不作首个 GA 门禁”的范围。
...
|---|---|---|
| `macos_codesign`、`macos_notarize` | macOS 发布者的 10 位 Apple Team ID | `AGENTGUARD_EXPECTED_MACOS_TEAM_ID` |
| `ios_codesign` | iOS 发布者的 10 位 Apple Team ID | `AGENTGUARD_EXPECTED_IOS_TEAM_ID` |
...
Windows 与 Android 指纹输入可含大小写或冒号，验证前会规范化为 64 位十六进制值；JSON 的 `signer`、命令输出里的证书身份和外部预期值必须指向同一个发布者。七个严格门禁验收 kind 不接受外部 signer。
...
模板只是字段清单，未填写时必须验证失败。执行真机验收，把报告保存为例如 `evidence/macos/report.md`。先实际运行人工验收校验并取得唯一 marker，再用同一入口计算报告闭包摘要；把精确命令、marker 与摘要填入 JSON 后显式复核：
...
其他十一类按上表替换 `kind` 与环境变量。不要手工把校验器本身、空目录或旧提交报告填进变量。
...
RC 严格门禁还要求 production preflight 零 `FAIL`。即使十二份 JSON 都通过，只要夹具密钥仍有生产语义、自动门禁失败，或签名/公证/真机/安装/升级/回滚材料不完整，结论仍是 **No-Go**。当前这些真实证据以及 GA 所需的 SBOM/许可证、隐私/商店声明、14 天 Beta、双 RC、五方签字、六渠道 smoke 和 5→25→100 扩量材料均未齐，因此当前仍是 **GA No-Go**。
