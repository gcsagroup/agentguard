...
const dictionaries = {
  en: {
    ...
    howtoS3why: "A high-risk action — a payment, a transfer, a privacy-trap form — raises a confirmation you must answer: “Deny and pause” holds it, “Allow once” lets that one action through. Two minutes with no answer counts as a refusal. Every verdict goes into the local audit trail (hash-chained, signable).",
  ...
  "zh-Hans": {
    ...
    howtoS3: "遇到风险时会发生什么",
    howtoS3why: "高危动作(付款、转账、隐私陷阱表单)会弹出一个必须回答的确认层:点「拒绝并暂停」就按住不放行,点「允许这一次」才放过这一次。两分钟没人回答按拒绝处理。每条判决都进本地审计(哈希链、可签名)。",
  ...
  "zh-Hant": {
    ...
    howtoS3: "遇到風險時會發生什麼",
    howtoS3why: "高危動作(付款、轉帳、隱私陷阱表單)會彈出一個必須回答的確認層:點「拒絕並暫停」就按住不放行,點「允許這一次」才放過這一次。兩分鐘沒人回答按拒絕處理。每條判決都進本機稽核(哈希鏈、可簽章)。",
