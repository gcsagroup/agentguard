...
In the preceding correction, the app and extension shared build number 2, retaining their names and bundle IDs. Removed in-page allow-once authorization and replay, and fixed a forged-overlay-attribute bypass. New events use `blocked`; historical `allowed` and `cancelled` retain their meaning. All four new extension regressions failed on the old code. The correction passed Node 22/22, Swift Core 21/21 and UI 2/2, plus strict Xcode 27 Release Simulator/unsigned-device builds and Analyze. Real Chromium DOM fixtures verified trilingual notices and zero risk requests using a synthetic native bridge. These do not replace device Safari or TestFlight acceptance. See the [record](../../docs/ios-block-only-2026-09-17.zh.md).

## 2026-09-17 keyboard notices (build 3)

In the preceding correction, the app and extension were 1.0.0 (3), retaining their names and bundle IDs. Risk dialogs expose localized names and descriptions, keep Tab/Shift+Tab on Close, and restore a still-connected prior control on Close or Escape. Stacked notices handle focus in order. No allow-once or replay was added. All four new regressions failed on the old source; the correction passed Node 26/26, Swift 23/23, both strict Release builds and Analyze. Real Chromium DOM keyboard checks in three languages produced no risk requests, using a synthetic native bridge. Device Safari, VoiceOver, Dynamic Type and TestFlight remain unverified. The preceding CI also has unresolved macOS startup-wait and Windows concurrent-audit failures. [Record](../../docs/ios-dialog-accessibility-2026-09-17.zh.md).
...
The preceding app and extension were 1.0.0 (22) with the same names and identifiers. [Build 21](../../docs/ios-page-audit-2026-09-17.zh.md) improved page layout and multiline text, but failed compilation under CI Xcode 16.4 because the new API is absent. Build 22 adds a compile-time guard and passes the complete local gate; CI for the exact correction commit `b458ca5` passed all 13 jobs, including Xcode 16.4/iOS 18.5 with Node 26/26, Swift 26/26, strict Release builds and Analyze. Maximum text size, device Safari, VoiceOver and TestFlight remain unverified. [Correction and evidence](../../docs/ios-sdk-compatibility-2026-09-17.zh.md).

## Contrast position controls and test reliability (2026-09-17)

The current app and extension are **1.0.0 (28)** with unchanged names and IDs. Checks reinstall this project's UI test runner after observed execution of old test logic, preserving product data. Three maximum-size labels produce 1/0/1 reports across clipped/visible/clipped positions; deliberately low contrast remains detected. Product UI and formal full-category audits are unchanged. Node 26/26, Swift 26/26, strict Release, Analyze and repository invariants 23/23 passed; six standard-size screenshots in three languages were reviewed. Baseline `afbd1e7` passed all 13 CI jobs. Full maximum-size acceptance, I6, device Safari, VoiceOver and TestFlight remain incomplete. [Evidence and limits](../../docs/ios-contrast-position-2026-09-17.zh.md).
