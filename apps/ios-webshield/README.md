...
**1.0.0 (34)**。构建 33 的准确 CI 为 12 项成功／1 项失败，繁中仍有无法定位的文字识别报告。构建 34 将文字识别独立到其它审计之前，并记录阶段树、截图和报告类型；全部类别和未知报告失败条件保持。

本机标准浅色完整检查通过：Node 26／26、Swift 26／26、严格 Release、Analyze 与仓库 23／23。深色及最大字号补充回归进行中，旧系统修复待新提交 CI；I6 和发布门槛未通过。 [记录](../../docs/ios-ci-audit-2026-09-17.zh.md)。
