...
DOM 保護涵蓋瀏覽器實際注入內容腳本的 HTTP(S) frame。可觀察的 click/submit，以及未改寫原型的 `form.submit()`，會在執行前被同步阻斷。頁面從 iframe 恢復原生方法、未注入的頁面／協定以及其他不產生被監聽事件的腳本路徑不在該保證內。網頁能破壞的是資訊提示的可見性或真實性，不能藉此產生放行狀態。
