...
const messages = {
  en: {
    ...
    "guard.resume": "Resume",
    "audit.decision.approve": "you allowed it once",
    "audit.decision.deny": "you held it",
    "audit.decision.timeout": "nobody answered in two minutes — treated as held",
    ...
    "howto.s3why": "A high-risk action — a payment, a transfer, a privacy-trap form — raises a confirmation you must answer: \u201cNot now — pause the task\u201d holds it, \u201cAllow once\u201d lets that one action through. Two minutes with no answer counts as a refusal. Every verdict goes into the local audit trail (hash-chained, signable).",
  ...
  "zh-Hans": {
    ...
    "guard.end": "结束守护",
    "guard.resume": "恢复暂停",
    "audit.decision.approve": "你允许了这一次",
    "audit.decision.deny": "你按住了它",
    "audit.decision.timeout": "两分钟没人回答,按拒绝处理",
    "dev.rawState": "原始状态行",
    "howto.title": "怎么用",
    ...
    "howto.s3": "遇到风险时会发生什么",
    "howto.s3why": "高危动作(付款、转账、隐私陷阱表单)会弹出一个必须回答的确认层:点「先不要，暂停任务」就按住不放行,点「允许这一次」才放过这一次。两分钟没人回答按拒绝处理。每条判决都进本地审计(哈希链、可签名)。",
  ...
  "zh-Hant": {
    ...
    "guard.end": "結束守護",
    "guard.resume": "恢復",
    "audit.decision.approve": "你允許了這一次",
    "audit.decision.deny": "你按住了它",
    "audit.decision.timeout": "兩分鐘沒人回答,按拒絕處理",
    "dev.rawState": "原始狀態行",
    "howto.title": "怎麼用",
    ...
    "howto.s3": "遇到風險時會發生什麼",
    "howto.s3why": "高危動作(付款、轉帳、隱私陷阱表單)會彈出一個必須回答的確認層:點「先不要，暫停任務」就按住不放行,點「允許這一次」才放過這一次。兩分鐘沒人回答按拒絕處理。每條判決都進本機稽核(哈希鏈、可簽章)。",
