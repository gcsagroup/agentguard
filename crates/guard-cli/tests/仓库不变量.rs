...
fn 前端不出现把字符串当代码的写法() {
    ...
    // 那个新文件有没有 sink。
    // 已检查 workspace、三语词表、gateway-confirmation、codex-setup、local-agent、knowledge、execution-records、gateway-governance、三个邮件模块/测试，以及 guard-native-submit.js 与 native-submit.test.mjs；正文不用代码 sink。
    const 前端文件数: usize = 33;
    assert_eq!(
        文件.len(),
        前端文件数,
