...
- 修复 Windows 默认主线程栈不足时 `guard-cli` 会在进入子命令前溢出的问题；Windows 入口现在以显式 8 MiB 栈运行同一 CLI 调度。发布门禁参数测试在 Windows 上明确调用 GitHub Runner 的 `gitbash.exe`，再由原生 `current_dir` 进入仓库并传入可移植相对路径，同时绑定脚本自己的退出码 2 与拒绝文本；WSL、路径或 CLI 启动失败都不能再冒充安全拒绝。
