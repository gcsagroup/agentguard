...
Current development supports protected local tasks with model-data consent, isolated snapshots, command and diff approval, verified host writeback, and persistent audit. Bounded acceptance also covers provenance and tool governance, [memory and delegation governance](docs/agd-026-memory-governance-2026-09-16.zh.md), and [limited media parsing](docs/agd-028-native-media-2026-09-16.zh.md). Memory, delegation, and media require explicit gateway configuration; the built-in model form does not automatically enable them. Linked acceptance records are in Simplified Chinese.

The fixed development app is **AgentGuard Local Agent Test, Ver 2.1 (024)**. Activity entries display timestamps, and passive observations of ordinary installation instructions and input fields no longer produce those false alerts. The [022 native record](docs/native-form-observation-022-2026-09-16.zh.md) and [window-read recovery record](docs/native-ax-recovery-022-2026-09-16.zh.md) define the exact candidate and tested scope. Its name, path, Bundle ID, and signing identity remain fixed; version numbers are not added to the filename.

023 includes the database fix in the desktop and bundled gateway; both permissions and historical records survived the update and restart. See the [023 native check](docs/native-sqlite-023-2026-09-16.zh.md).

024 fixes installation-risk prompts during ordinary Safari PR browsing, with checks of the original page and a positive injection case; see the [024 native record](docs/native-pr-observation-024-2026-09-16.zh.md). See the [iOS page-audit record](docs/ios-page-audit-2026-09-17.zh.md) for the current accessibility changes and verification boundaries.

**The full plan remains incomplete.** On 2026-09-22 Chrome candidate-ZIP B1/B2/B3/B5 were completed (B4 N/A, Edge missing), independent desktop injection pauses the same-session gateway, and the active-protection page distinguishes the three paths; see the [first-wave record](docs/agd-active-defense-2026-09-22.zh.md). AGD-027 is closed under the agreed budget split: native read p95 ≤20 ms, first start recorded only; see the [budget-split record](docs/agd-027-budget-split-2026-09-22.zh.md). The original 100 ms failures are retained. F13 remains deferred and unverified at the user's request. AGD-031 has completed local STIX round trips without external delivery; AGD-032 awaits production-release evidence. The [development plan](docs/agentguard-development-plan-2026-09-09.zh.md) is the source for current task status and individual evidence; neither early 010 results nor later individual checks establish complete M1 or release acceptance.
...
- The first-GA browser package supports Chrome / Edge only. Firefox remains a source prototype and is neither packaged nor an acceptance gate. iOS WebShield/Safari Extension is a separate Xcode/Swift limited SKU in the first-release product scope, not a future option. The fixed macOS development app 024 has an organizational signature and bounded native acceptance; it does not replace production universal-package signing, notarization, or install/upgrade/uninstall acceptance. Historical Windows candidate `89dadf9` has partial evidence, while current signed installers and real-device acceptance remain open. iOS builds and has unsigned Simulator evidence, but is not wired to the Rust engine; production signing, real-device Safari, and TestFlight remain open.
