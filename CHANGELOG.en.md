...
- Fixed `guard-cli` overflowing the default Windows main-thread stack before entering a subcommand. The Windows entry point now runs the same CLI dispatch with an explicit 8 MiB stack. Release-gate argument tests now enter the repository through the native `current_dir` API and give Git Bash only a portable relative path, ensuring they reach the script's exit-code-2 branch; negative CLI tests bind the expected rejection reason so a startup crash cannot masquerade as a security rejection.
