...
- Fixed `guard-cli` overflowing the default Windows main-thread stack before entering a subcommand. The Windows entry point now runs the same CLI dispatch with an explicit 8 MiB stack. On Windows, release-gate argument tests now explicitly invoke GitHub Runner's `gitbash.exe`, enter the repository through the native `current_dir` API, and pass only a portable relative path while binding both the script's exit code 2 and its rejection text; WSL, path, or CLI startup failures can no longer masquerade as security rejections.
