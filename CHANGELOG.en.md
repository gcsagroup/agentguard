...
- Fixed `guard-cli` overflowing the default Windows main-thread stack before entering a subcommand. The Windows entry point now runs the same CLI dispatch with an explicit 8 MiB stack. Release-gate argument tests also use a forward-slash repository path that Git Bash can execute, ensuring they reach the script's exit-code-2 branch; negative CLI tests now bind the expected rejection reason so a startup crash cannot masquerade as a security rejection.
