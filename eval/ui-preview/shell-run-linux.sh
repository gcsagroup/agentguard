#!/usr/bin/env bash
# 把 macOS 桌面壳子**真的跑起来**(Linux / WebKitGTK,无头 Xvfb),点一遍主路径并截图。
#
# # 为什么有这个脚本
#
...
# 用法:bash eval/ui-preview/shell-run-linux.sh [--keep]
set -euo pipefail
...
echo "== 1/4 编译壳子(Linux 目标)"
(cd "$REPO/apps/desktop-macos/src-tauri" && cargo build --bin desktop-macos)
...
WEBKIT_DISABLE_COMPOSITING_MODE=1 "$REPO/apps/desktop-macos/src-tauri/target/debug/desktop-macos" \
  >/tmp/agentguard-shell.log 2>&1 &
APP_PID=$!
cleanup() {
  if [[ "$KEEP" -eq 0 ]]; then
...
for _ in $(seq 1 30); do
  WIN="$(xdotool search --name 'AgentGuard macOS' 2>/dev/null | head -1 || true)"
  [[ -n "$WIN" ]] && break
  sleep 1
done
...
sleep 3
import -window root "$OUT/linux-run-1-firstrun.png"

echo "== 3/4 点主路径:开始守护 → 自检 → 先不要 → 结束守护"
click() { xdotool mousemove "$1" "$2" click 1; sleep "${3:-3}"; }
click 227 936        # 开始守护
import -window root "$OUT/linux-run-2-started.png"
click 331 777        # 自检:模拟一次付款拦截
import -window root "$OUT/linux-run-3-confirm.png"
click 511 603        # 先不要 —— 暂停这项任务
xdotool mousemove 590 900; xdotool click --repeat 8 5; sleep 2
import -window root "$OUT/linux-run-4-timeline.png"
xdotool mousemove 590 500; xdotool click --repeat 10 4; sleep 2
click 375 936        # 结束守护
import -window root "$OUT/linux-run-5-stopped.png"

echo "== 4/4 结果"
echo "截图:$OUT/linux-run-{1..5}-*.png"
