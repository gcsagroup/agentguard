...
- Firefox 移植與 Edge 相容路徑已具備，但尚無真實瀏覽器端對端驗收；Safari 目前只有設計。目前 macOS ad-hoc 候選已在本機通過啟動、TCC 探測與 AXObserver 推送流程檢查，但尚未完成簽署/公證後的全新安裝與升級驗收。Windows 候選 `89dadf9` 已取得真實 Windows 11 上的啟動、連續觀測與阻斷模態部分證據，但 W1–W7 清單、簽署安裝套件及全新安裝/升級/解除安裝仍未完成；iOS 仍是未接入引擎的有限骨架。
...
**裝好之後怎麼用**(授權哪兩項、「開始守護」到底做了什麼、狀態燈各是什麼意思、怎麼用自我檢查在 5 秒內
看到它攔一次、什麼都沒發生時按什麼順序查):見 [桌面端使用說明](docs/desktop-guide.zh-TW.md)。
應用主介面裡也有同一套三步速覽。
